...
export default function StudentSession() {
  ...
  if (isSessionLoading)
    return <div className="p-8 text-center">Entering classroom...</div>;
  if (!verifyData.valid)
          ...
          {questions && questions.length > 0 ? (
            questions.map((q) => (
              <Card
                key={q.question_id}
                className={
                  q.student_hash === studentHash ? 'border-primary' : ''
                }
              >
                <CardHeader className="pb-2 p-4">
                  <div className="flex justify-between items-start">
                    <span
                      className={`text-sm font-semibold ${q.student_hash === studentHash ? 'text-primary' : 'text-muted-foreground'}`}
                    >
                ...
                <CardContent className="p-4 pt-0">
                  ...
                  {q.status === 'flagged' ? (
                    ...
                    <div className="bg-secondary/50 p-3 rounded text-sm">
                      <span className="font-bold text-primary text-xs uppercase block mb-1">
                        AI Answer
                      </span>
                      {q.ai_response}
                    </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSend} className="flex gap-2">
            ...
            <Button
              type="submit"
              size="icon"
              disabled={postQuestion.isPending || !questionText.trim()}
            >
